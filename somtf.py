...
class SOM(object):
    def __init__(self, x, y, input_dim, learning_rate, radius, num_iter=111):
        ...
        with self._graph.as_default():
            ...
            self._weights = tf.Variable(tf.compat.v1.random_normal([x * y, input_dim]))
            ...
            neighbourhood_func = tf.exp(
                tf.negative(tf.math.divide(tf.cast(bmu_distance, "float32"), tf.pow(_current_radius, 2))))
    ...
    def train(self, input_vectors, input_classes):
        for iter_no in range(self._num_iter):
            for input_vect in input_vectors:
        ...
        input_locations = self.map_input(input_vectors)
        flat_input_classes = [item for sublist in input_classes for item in sublist]
        for location_index in range(self._locations.__len__()):
            indices = [i for i, x in enumerate(input_locations) if x == location_index]
            location_classes = [flat_input_classes[i] for i in indices]
            if not location_classes:
                self._location_classes[location_index] = -1
                continue
            most_frequent_class = max(set(location_classes), key = location_classes.count)
            self._location_classes[location_index] = most_frequent_class

    def predict(self, input_vectors):
        input_locations = self.map_input(input_vectors)
        return list(map(self._location_classes.get, input_locations))
